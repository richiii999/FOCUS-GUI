...
if input("Test2: Verifying it came from the KB (y/N): ") == 'y':
    with open('./Prompts/Para/GenerateActions.txt', 'r') as f1:
        ...
        prompt += "At the end, please cite information about the user as to why you chose these particular actions"

        TempContext = [{"role":"user", "content":prompt}]

    response = API.chat_with_collection(API.Models[SLM.modelNum], TempContext, API.KBIDs[0])['choices'][0]['message']['content']
